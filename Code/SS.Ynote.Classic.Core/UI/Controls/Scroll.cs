...
namespace SS.Ynote.Classic.Core.UI
{
    public class MyScrollBar : Control
    {
        public UITheme Theme
        {
            set
            {
                var tC = value.GetThemeClass("scroll_bar");
                this.BackColor = value.ToColor(tC["background"]);
                this.BorderColor = value.ToColor(tC["border_color"]);
                this.ThumbColor  = value.ToColor(tC["thumb_color"]);
                this.ThumbSize  = (int)tC["thumb_size"];
            }
        }
        private int @value;
        ...
        private void MouseScroll(MouseEventArgs e)
        {
            ...
            switch (Orientation)
            {
                ...
                case ScrollOrientation.HorizontalScroll:
                    v = Maximum*(e.X - thumbSize/2)/(Width - thumbSize);
                    break;
            }
